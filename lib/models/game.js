...
const checkMaxStocks = function(corpStocks) {
...
class Game {
  ...
  updateStocksAfterBuy(corpStocks, playerId) {
    for (const corp in corpStocks) {
      if (corpStocks[corp]) {
        this.corporations.removeStocks(corp, corpStocks[corp]);
        const player = this.getPlayer(playerId);
        player.addStocks(corp, corpStocks[corp]);
      }
    }
  }

  buyStocks(corpStocks, playerId) {
    const totalStocks = checkMaxStocks(corpStocks);
    if (totalStocks > 3) {
      return false;
    }
    this.updateStocksAfterBuy(corpStocks, playerId);
  ...
  getStateData(playerId) {
    ...
    if (state === 'buyStocks') {
      data.activeCorps = this.corporations.getActiveCorporate();
    }
    return data;
  }
