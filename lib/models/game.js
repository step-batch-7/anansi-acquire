...
class Game {
  ...
  changePlayerTurn() {
    ...
    this.currentPlayer.statusMsg = this.currentPlayer.status + msg;
  ...
  updateActivity(statusMsg, activityMsg, type) {
    this.currentPlayer.statusMsg = statusMsg;
    this.activityLog.addLog(type, activityMsg);
  }

  updateActivityForTilePlaced(tile) {
    const tileName = tileGenerator(tile);
    const statusMsg = `You placed ${tileName}. `;
    const name = this.currentPlayer.playerName;
    const activityMsg = `${name} placed ${tileName}. `;
    this.updateActivity(statusMsg, activityMsg, 'tilePlaced');
  }

  canPlayerPlaceTile(tile) {
    const state = this.currentPlayer.state;
    return state === 'placeTile' && this.currentPlayer.removeTile(tile);
  }
  ...
  getTileType(tile){
    ...
    if(adjacentCorp.length > 1){
      result.type = 'merger';
      result.corporations = adjacentCorp;
      return result;
    }
    return result;
  }
  ...
  manageTilePlacement(tile){
    const result = this.getTileType(tile);
    if(result.type == 'place'){
      return this.placeNormalTile(tile);
    }
    return this.mergeCorporations(tile, result.corporations);
  }
  ...
  mergeCorporations(tile, corporations){
    ...
    const sortedCorp = lodash.unzip(sortedPair)[1];
    this.corporations.mergeCorporate(...sortedCorp, tile);
  }

  placeATile(tile) {
    if (this.canPlayerPlaceTile(tile)) {
  ...
  updateActivityAfterEstablish(corporation) {
    ...
    const msg = `You established ${corporation}. `;
    this.updateActivity(msg, activityMsg, 'establish');
  }
