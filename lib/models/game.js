...
const getMaxAndSecondMaxNumbers = function(numbers) {
  ...
  for (let index = 0; index < sortedList.length; index++) {
    if (sortedList[index] === max) {
      break;
    }
    if (sortedList[index] < max) {
      secondMax = sortedList[index];
      break;
    }
  }
...
=======
    if(length === 1){
>>>>>>> |#9| Rivu/Gulshan| added distributeBonus feature in game class
      return this.placeNormalTile(tile);
    }
  ...
  distributeBonus(bonus, smallCorp){
    ...
    if(!secondMaxStock){
      return (
        this.giveBonusToMaxStockHolders(bonus, maxStock, smallCorp)
      );
    }
    giveBonusToStockHolders(bonus, stocks, smallCorp);
  }
