...
const placeATile = function(req, res){
  const {tile} = req.body;
  const {gameId, playerId} = req.player;
  const {games} = req.app.locals;
  games[gameId].placeATile(tile);
  res.json(games[gameId].getStatus(playerId));
};

const getGameStatus = function(req, res) {
  const {gameId, playerId} = req.player;
  const {games} = req.app.locals;
  games[gameId].setCurrentPlayerStatus();
  res.json(games[gameId].getStatus(playerId));
};
...
const serveGameIdAndPlayers = function(req, res) {
  res.json({gameId: req.player.gameId});
};
