...
const serveGameIdAndPlayers = function(req, res) {
  ...
  const [hosted] = req.game.players;
  const joined = req.game.players.slice(NUMBER).map(player => player.name);
  const remaining = req.game.noOfPlayers - req.game.players.length;
  res.json({hasJoined, gameId, hosted: hosted.name, joined, remaining});
};
